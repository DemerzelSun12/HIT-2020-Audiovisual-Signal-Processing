...
class LocatePlate(object):
    ...
    def binaryzation(self):
        ...
        x = maxi - ((maxi - mini) / 2)
        ret, thresh = cv.threshold(self.hat_img, x, 255, cv.THRESH_BINARY)
        return thresh
    ...
    def filter_img(self):
        m = 400 * self.img.shape[0] / self.img.shape[1]
        self.img = cv.resize(self.img, (400, int(m)), interpolation=cv.INTER_AREA)
    ...
    def detect_edge(self):
        contours, hierarchy = cv.findContours(self.open_img, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
        block = []
        for c in contours:
